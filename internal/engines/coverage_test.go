...
func TestCheckEngineCoverage(t *testing.T) {
	...
	// 'admin' (repository#edit.op.1) should be uncovered because of short-circuit (owner was true)
	foundAdmin := false
	for _, node := range report {
		if node.Path == "repository#edit.op.1" {
			foundAdmin = true
			break
		}
	}
	...
	if !foundAdmin {
		t.Errorf("expected repository#edit.op.1 (admin) to be uncovered due to short-circuit, but it wasn't in the report")
	}
}
